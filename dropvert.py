...
if leftdjfp1 is not None:
    ...
    graph.add_trace(go.Scatter(x=dfldj1['Time'], y= dfldj1['Fz'], line=dict(color='red')))
if rightdjfp1 is not None:
    dfrdj1= pd.read_csv(rightdjfp1, header =(0),sep='\t')
    graph.add_trace(go.Scatter(x=dfrdj1['Time'], y= dfrdj1['Fz'], line=dict(color='green')))
...
if rightdjfp1 is not None and leftdjfp1 is not None:
    ...
    if bwkgs !=0:
        ...
        netImpulseRInterval= dfrdj1['Fz'][righttouchdown:righttakeoff] - (bwkgs*9.81/2)
        netImpulseTimeR= dfrdj1['Time'][righttouchdown:righttakeoff]
        netImpulseLInterval= dfldj1['Fz'][lefttouchdown:lefttakeoff] - (bwkgs*9.81/2)
        ...
        if zeroVelocityTime != 0:
            ...
            concentricImpulseTimeLInterval= dfldj1['Time'][zeroVelocityTime:lefttakeoff]

            concentricImpulseR=integrate.simps(concentricImpulseRInterval,concentricImpulseTimeR)
            concentricImpulseL=integrate.simps(concentricImpulseLInterval,concentricImpulseTimeLInterval)
            ...
            eccentricImpulseTimeLInterval= dfldj1['Time'][lefttouchdown:zeroVelocityTime]

            eccentricImpulseR=integrate.simps(eccentricImpulseRInterval,eccentricImpulseTimeR)
            eccentricImpulseL=integrate.simps(eccentricImpulseLInterval,eccentricImpulseTimeLInterval)
    ...
    df= pd.DataFrame(data, columns=['Name','Weight-kg','Zero Velocity Time','Net Impulse-L', 'Net Impulse-R'])
